...
router.patch("/Transfer/:id", async (req, res) => {
  ...
  //   return res.status(400).send({ error: "Invalid Updates" });
  // }
    const {CloserStatus,AgentStatus,AdminStatus,Status} = req.body
  try {
    const sale = await Sale.findByIdAndUpdate(req.params.id, { CloserStatus, AgentStatus,AdminStatus,Status});
    // const user = await User.findByIdAndUpdate(req.params.id,req.body,{ new:true , runValidators:true})
...
router.patch("/KickBack/:id", async (req, res) => {
  ...
  //   return res.status(400).send({ error: "Invalid Updates" });
  // }
    const {CloserStatus,AgentStatus,AdminStatus,Status, AgentId} = req.body
  try {
    const sale = await Sale.findByIdAndUpdate(req.params.id, { CloserStatus, AgentStatus,AdminStatus,Status ,AgentId});
    // const user = await User.findByIdAndUpdate(req.params.id,req.body,{ new:true , runValidators:true})
